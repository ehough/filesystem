...
class ehough_filesystem_Filesystem implements ehough_filesystem_FilesystemInterface
{
    ...
    public function copy($originFile, $targetFile, $override = false)
    {
        ...
        if ($doCopy) {
            ...
            if (false === $source = @fopen($originFile, 'r')) {
                throw new ehough_filesystem_exception_IOException(sprintf('Failed to copy "%s" to "%s" because source file could not be opened for reading.', $originFile, $targetFile), 0, null, $originFile);
            }

            // Stream context created to allow files overwrite when using FTP stream wrapper - disabled by default
            if (false === $target = @fopen($targetFile, 'w', null, stream_context_create(array('ftp' => array('overwrite' => true))))) {
                throw new ehough_filesystem_exception_IOException(sprintf('Failed to copy "%s" to "%s" because target file could not be opened for writing.', $originFile, $targetFile), 0, null, $originFile);
            }
            ...
            if (stream_is_local($originFile) && $bytesCopied !== ($bytesOrigin = filesize($originFile))) {
                throw new ehough_filesystem_exception_IOException(sprintf('Failed to copy the whole content of "%s" to "%s" (%g of %g bytes copied).', $originFile, $targetFile, $bytesCopied, $bytesOrigin), 0, null, $originFile);
            }
        }
    }
    ...
    public function tempnam($dir, $prefix)
    {
        ...
        if (null === $scheme || 'file' === $scheme) {
            ...
            throw new ehough_filesystem_exception_IOException('A temporary file could not be created.');
        }
